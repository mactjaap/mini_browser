...
def phase2_link_limits(badge):
    ...
    # The serial console can interleave the runner's own [MATCH] output with
    # CONTENT START/END text. The browser's parser diagnostic above is the
    # authoritative MAX_LINKS/MAX_ACTIONS boundary check. Keep one negative
    # rendering check to ensure the 129th input link did not receive an action.
    if re.search(r"\[129\]LimitLink129", content):
        raise RuntimeError("LimitLink129 was incorrectly assigned action 129")
