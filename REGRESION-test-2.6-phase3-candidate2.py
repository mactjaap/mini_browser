...
"""
Configurable Mini Browser 2.6 / WHY2025 BadgeVMS regression tester.
...
CONFIG = {
    ...
    # Upload phase3-image-poc.html and phase3-demo.ppm to the document root.
    "phase3jpeg": {
        "url": "minibrowser.macip.net/phase3-jpeg.html",
        "url_pattern": r"HTTP 200.*https://minibrowser\.macip\.net/phase3-jpeg\.html",
    },
    "phase3png": {
        "url": "minibrowser.macip.net/phase3-png.html",
        "url_pattern": r"HTTP 200.*https://minibrowser\.macip\.net/phase3-png\.html",
    },
...
def phase3_jpeg_image(badge):
    open_direct_url(badge, CONFIG["phase3jpeg"]["url"], CONFIG["phase3jpeg"]["url_pattern"])
    badge.wait_for(
        r"\[mini_browser\] display: mode=Colors \+ Images images_seen=1 images_retained=1 image_loaded=1",
        20,
    )
    badge.wait_for(r"\[mini_browser\] image: loaded 160x100, compressed=", 5)
    content = latest_content_block(badge)
    if not content:
        raise RuntimeError("Phase 3 JPEG page loaded but no CONTENT block was captured")
    require_content(content,
                    "PHASE3 JPEG TEST",
                    "Text BEFORE the JPEG.",
                    "Text AFTER the JPEG - must be below it.",
                    "END PHASE3 JPEG TEST")
    return ["JPEG downloaded and decoded from memory",
            "Image dimensions accepted before full decode",
            "Surrounding styled text preserved"]


def phase3_png_image(badge):
    open_direct_url(badge, CONFIG["phase3png"]["url"], CONFIG["phase3png"]["url_pattern"])
    badge.wait_for(
        r"\[mini_browser\] display: mode=Colors \+ Images images_seen=1 images_retained=1 image_loaded=1",
        20,
    )
    ...
    content = latest_content_block(badge)
    if not content:
        raise RuntimeError("Phase 3 PNG page loaded but no CONTENT block was captured")
    require_content(content,
                    "PHASE3 PNG TEST",
                    "Text BEFORE the PNG.",
                    "Text AFTER the PNG - must be below it.",
                    "END PHASE3 PNG TEST")
    return ["PNG downloaded and decoded from memory",
            "Image dimensions accepted before full decode",
            "Surrounding styled text preserved"]
...
def main():
    ...
    try:
        ...
        # Mini Browser 2.6 Phase 3: real JPEG/PNG image decoding.
        run_test(
            results,
            number,
            "2.6 Phase 3: JPEG image decode",
            lambda: phase3_jpeg_image(badge),
        )
        number += 1
        ...
        run_test(
            results,
            number,
            "2.6 Phase 3: PNG image decode",
            lambda: phase3_png_image(badge),
        )
        number += 1
