...
CONFIG = {
    ...
    "phase3poc": {
        "url": "minibrowser.macip.net/phase3-image-poc.html",
        "url_pattern": r"HTTP 200.*https://minibrowser\.macip\.net/phase3-image-poc\.html",
    },
...
def phase3_display_modes_image_poc(badge):
    open_direct_url(badge, CONFIG["phase3poc"]["url"], CONFIG["phase3poc"]["url_pattern"])
    badge.wait_for(
        r"\[mini_browser\] display: mode=Colors \+ Images images_seen=1 images_retained=1 image_loaded=1",
        20,
    )
    badge.wait_for(r"\[mini_browser\] image: loaded PPM 80x60,", 5)
    content = latest_content_block(badge)
    if not content:
        raise RuntimeError("Phase 3 POC page loaded but no CONTENT block was captured")
    require_content(content,
                    "PHASE3 IMAGE POC",
                    "This page proves display modes and one bounded image.",
                    "Yellow bold survives in color modes.",
                    "White text on dark blue.",
                    "Use WHY+O and select 1, 2 or 3.",
                    "END PHASE3 IMAGE POC")
    return ["One <img> retained in Colors + Images mode",
            "Bounded PPM image downloaded and decoded",
            "Phase 1B/2A/2B styled content preserved"]
...
def main():
    ...
    try:
        ...
        # Mini Browser 2.6 Phase 3: display modes + one bounded image POC.
        run_test(
            results,
            number,
            "2.6 Phase 3 POC: display modes and one image",
            lambda: phase3_display_modes_image_poc(badge),
        )
        number += 1
