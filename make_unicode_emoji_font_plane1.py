...
BMP_FONT = sys.argv[1] if len(sys.argv) > 1 else "unifont_jp-17.0.02.hex"
UPPER_FONT = sys.argv[2] if len(sys.argv) > 2 else "unifont_upper-17.0.02.hex"
OUT = sys.argv[3] if len(sys.argv) > 3 else "unifont_cjk.bin"
...
def load_hex_font(filename):
    ...
    source_counts[filename] = (loaded, usable)

load_hex_font(BMP_FONT)
load_hex_font(UPPER_FONT)
...
print("Mini Browser extended Unicode + Plane 1 font")
print()
print("Sources:")
for filename, (loaded, usable) in source_counts.items():
    print(f"  {filename}: {loaded} selected code points, {usable} usable glyphs")
print()
