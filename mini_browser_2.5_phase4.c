...
/* Mini Browser 2.5 Phase 4: deliberately bounded, in-memory cookie jar. */
#define MAX_COOKIES          16
#define COOKIE_NAME_MAX      32
#define COOKIE_VALUE_MAX    128
#define COOKIE_DOMAIN_MAX   128
#define COOKIE_PATH_MAX     128
#define COOKIE_HEADER_MAX  2048
...
/* ---------- Mini Browser 2.5 Phase 4: bounded cookie jar ---------- */
...
typedef struct {
    char request_url[URL_MAX];
} cookie_header_ctx_t;

static cookie_t g_cookies[MAX_COOKIES];
static unsigned long g_cookie_serial = 1;

static void cookie_copy_lower(char *dst, size_t cap, const char *src, size_t n) {
    if (!dst || cap == 0) return;
...
static void cookie_copy_trimmed(char *dst, size_t cap, const char *src, size_t n) {
    if (!dst || cap == 0) return;
    if (!src) { dst[0] = 0; return; }

    while (n > 0 && isspace((unsigned char)*src)) {
        src++;
        n--;
    }
...
static bool cookie_url_parts(const char *url,
                             char *host, size_t host_cap,
                             char *path, size_t path_cap,
                             bool *secure) {
    if (!url || !host || host_cap == 0 || !path || path_cap == 0)
        return false;
    ...
    cookie_copy_lower(host, host_cap, host_start,
    ...
    if (*authority_end == '/') {
        ...
        while (*path_end && *path_end != '?' && *path_end != '#')
            path_end++;
        size_t n = (size_t)(path_end - authority_end);
        if (n >= path_cap) n = path_cap - 1;
        memcpy(path, authority_end, n);
        path[n] = 0;
    } else {
...
static void cookie_default_path(const char *request_path,
    ...
    if (!last || last == request_path) {
        strncpy(out, "/", cap);
        out[cap - 1] = 0;
        return;
    }

    size_t n = (size_t)(last - request_path);
    if (n >= cap) n = cap - 1;
    memcpy(out, request_path, n);
    out[n] = 0;
}
...
static bool cookie_path_match(const char *request_path,
                              const char *cookie_path) {
    if (!request_path || !*request_path) request_path = "/";
    if (!cookie_path || !*cookie_path) cookie_path = "/";

    size_t pl = strlen(cookie_path);
    if (strncmp(request_path, cookie_path, pl) != 0)
        return false;

    if (request_path[pl] == 0)
        return true;
    if (cookie_path[pl - 1] == '/')
        return true;
    return request_path[pl] == '/';
}

static int cookie_find_exact(const char *name,
                             const char *domain,
                             const char *path) {
    for (int i = 0; i < MAX_COOKIES; i++) {
        if (!g_cookies[i].used) continue;
        if (!strcmp(g_cookies[i].name, name) &&
            !strcasecmp(g_cookies[i].domain, domain) &&
            !strcmp(g_cookies[i].path, path))
            return i;
    }
    return -1;
}

static void cookie_delete_exact(const char *name,
                                const char *domain,
                                const char *path) {
    int i = cookie_find_exact(name, domain, path);
    if (i >= 0)
        memset(&g_cookies[i], 0, sizeof(g_cookies[i]));
}

static int cookie_slot_for_store(const char *name,
                                 const char *domain,
                                 const char *path) {
    int exact = cookie_find_exact(name, domain, path);
    ...
    /* Deterministic bounded eviction: replace the oldest stored cookie. */
    int oldest = 0;
    for (int i = 1; i < MAX_COOKIES; i++)
        if (g_cookies[i].serial < g_cookies[oldest].serial)
            oldest = i;
    return oldest;
}
...
static int cookie_count(void) {
    int count = 0;
    for (int i = 0; i < MAX_COOKIES; i++)
        if (g_cookies[i].used) count++;
    return count;
}

static void cookie_store_set_cookie(const char *request_url,
                                    const char *header_value) {
    char request_host[COOKIE_DOMAIN_MAX + 1];
    char request_path[COOKIE_PATH_MAX + 1];
    bool request_secure = false;

    if (!cookie_url_parts(request_url,
    ...
    char line[512];
    size_t line_len = strlen(header_value ? header_value : "");
    if (line_len >= sizeof(line))
        line_len = sizeof(line) - 1;
    memcpy(line, header_value ? header_value : "", line_len);
    line[line_len] = 0;
    ...
    char *pair_end = semi ? semi : line + strlen(line);
    char *eq = memchr(line, '=', (size_t)(pair_end - line));
    if (!eq) return;

    char name[COOKIE_NAME_MAX + 1];
    char value[COOKIE_VALUE_MAX + 1];
    cookie_copy_trimmed(name, sizeof(name), line, (size_t)(eq - line));
    cookie_copy_trimmed(value, sizeof(value), eq + 1,
    ...
    char domain[COOKIE_DOMAIN_MAX + 1];
    strncpy(domain, request_host, sizeof(domain));
    domain[sizeof(domain) - 1] = 0;
    bool host_only = true;

    char path[COOKIE_PATH_MAX + 1];
    cookie_default_path(request_path, path, sizeof(path));

    bool secure = false;
    bool delete_cookie = false;

    char *attr = semi ? semi + 1 : NULL;
    while (attr && *attr) {
        while (*attr == ';' || isspace((unsigned char)*attr)) attr++;
        if (!*attr) break;

        char *next = strchr(attr, ';');
        char *end = next ? next : attr + strlen(attr);
        char *attr_eq = memchr(attr, '=', (size_t)(end - attr));

        char attr_name[32];
        char attr_value[COOKIE_PATH_MAX + 1];

        if (attr_eq) {
            cookie_copy_lower(attr_name, sizeof(attr_name),
                              attr, (size_t)(attr_eq - attr));
            cookie_copy_trimmed(attr_value, sizeof(attr_value),
                                attr_eq + 1,
                                (size_t)(end - (attr_eq + 1)));
        } else {
            cookie_copy_lower(attr_name, sizeof(attr_name),
                              attr, (size_t)(end - attr));
            attr_value[0] = 0;
        }

        if (!strcmp(attr_name, "domain") && attr_value[0]) {
            const char *d = attr_value;
            while (*d == '.') d++;
            char candidate[COOKIE_DOMAIN_MAX + 1];
            cookie_copy_lower(candidate, sizeof(candidate), d, strlen(d));

            /* Reject a Domain attribute that does not cover the origin host. */
            if (!cookie_domain_match(request_host, candidate))
                return;

            strncpy(domain, candidate, sizeof(domain));
            domain[sizeof(domain) - 1] = 0;
            host_only = false;
        } else if (!strcmp(attr_name, "path") &&
                   attr_value[0] == '/') {
            strncpy(path, attr_value, sizeof(path));
            path[sizeof(path) - 1] = 0;
        } else if (!strcmp(attr_name, "secure")) {
            secure = true;
        } else if (!strcmp(attr_name, "max-age") && attr_value[0]) {
            char *endptr = NULL;
            long age = strtol(attr_value, &endptr, 10);
            if (endptr != attr_value && age <= 0)
                delete_cookie = true;
        }
    ...
    if (delete_cookie) {
        cookie_delete_exact(name, domain, path);
        printf("[mini_browser] cookie delete: %s domain=%s path=%s count=%d\n",
               name, domain, path, cookie_count());
        return;
    }

    int slot = cookie_slot_for_store(name, domain, path);
    cookie_t *c = &g_cookies[slot];
    memset(c, 0, sizeof(*c));
    c->used = true;
    c->secure = secure;
    c->host_only = host_only;
    c->serial = g_cookie_serial++;

    strncpy(c->name, name, sizeof(c->name));
    c->name[sizeof(c->name) - 1] = 0;
    strncpy(c->value, value, sizeof(c->value));
    c->value[sizeof(c->value) - 1] = 0;
    strncpy(c->domain, domain, sizeof(c->domain));
    c->domain[sizeof(c->domain) - 1] = 0;
    strncpy(c->path, path, sizeof(c->path));
    c->path[sizeof(c->path) - 1] = 0;

    printf("[mini_browser] cookie store: %s domain=%s path=%s secure=%d count=%d\n",
           c->name, c->domain, c->path, c->secure ? 1 : 0, cookie_count());
}

static bool cookie_build_header(const char *url,
                                char *out, size_t cap) {
    if (!out || cap == 0) return false;
    out[0] = 0;

    char host[COOKIE_DOMAIN_MAX + 1];
    char path[COOKIE_PATH_MAX + 1];
    bool secure = false;
    if (!cookie_url_parts(url, host, sizeof(host), path, sizeof(path), &secure))
        return false;
    ...
    for (int i = 0; i < MAX_COOKIES; i++) {
        const cookie_t *c = &g_cookies[i];
        if (!c->used) continue;
        if (c->secure && !secure) continue;

        bool domain_ok = c->host_only
            ? !strcasecmp(host, c->domain)
            : cookie_domain_match(host, c->domain);
        ...
        memcpy(out + used, c->name, n);
        used += n;
        out[used++] = '=';
        n = strlen(c->value);
        memcpy(out + used, c->value, n);
        used += n;
        out[used] = 0;
        any = true;
    }
...
static size_t cookie_header_cb(char *buffer, size_t size,
    ...
    cookie_header_ctx_t *ctx = (cookie_header_ctx_t *)userdata;
    ...
    if (n >= prefix_len &&
        !strncasecmp(buffer, prefix, prefix_len)) {
        char value[512];
        size_t value_len = n - prefix_len;
        if (value_len >= sizeof(value))
            value_len = sizeof(value) - 1;
        memcpy(value, buffer + prefix_len, value_len);
        value[value_len] = 0;
        cookie_store_set_cookie(ctx->request_url, value);
    }
...
static int fetch_url(const char *url, const char *post_body, mem_t *m, long *http_status) {
    ...
    char cookie_value[COOKIE_HEADER_MAX];
    char cookie_line[COOKIE_HEADER_MAX + 16];
    if (cookie_build_header(url, cookie_value, sizeof(cookie_value))) {
        snprintf(cookie_line, sizeof(cookie_line), "Cookie: %s", cookie_value);
    ...
    cookie_header_ctx_t cookie_ctx;
    strncpy(cookie_ctx.request_url, url, sizeof(cookie_ctx.request_url));
    cookie_ctx.request_url[sizeof(cookie_ctx.request_url) - 1] = 0;

    curl_easy_setopt(curl, CURLOPT_HEADERFUNCTION, cookie_header_cb);
    curl_easy_setopt(curl, CURLOPT_HEADERDATA, &cookie_ctx);
