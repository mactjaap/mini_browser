...
static size_t cookie_header_cb(char *buffer, size_t size,
    ...
    static const char cookie_prefix[] = "Set-Cookie:";
    static const char location_prefix[] = "Location:";
    const size_t cookie_plen = sizeof(cookie_prefix) - 1;
    const size_t location_plen = sizeof(location_prefix) - 1;

    /*
     * Phase 5: observe redirect response headers ourselves because the
     * trimmed BadgeVMS libcurl does not reliably expose redirect metadata.
     * A new HTTP status line starts each response in a redirect chain.
     */
    if (buffer && n >= 5 && !strncasecmp(buffer, "HTTP/", 5)) {
        const char *p = buffer;
        const char *end = buffer + n;

        g_fetch_meta.response_is_redirect = false;
        ...
        if (p + 2 < end &&
            ...
            p[1] >= '0' && p[1] <= '9' &&
            p[2] >= '0' && p[2] <= '9') {
            g_fetch_meta.response_is_redirect = true;
        }
    }

    if (buffer && n >= location_plen &&
        !strncasecmp(buffer, location_prefix, location_plen) &&
        g_fetch_meta.response_is_redirect) {
        g_fetch_meta.redirect_seen = true;
    }

    if (buffer && n >= cookie_plen &&
        !strncasecmp(buffer, cookie_prefix, cookie_plen)) {
        size_t vlen = n - cookie_plen;
        if (vlen >= sizeof(g_cookie_set_line))
            vlen = sizeof(g_cookie_set_line) - 1;
        memcpy(g_cookie_set_line, buffer + cookie_plen, vlen);
...
static int fetch_url(const char *url, const char *post_body, mem_t *m, long *http_status) {
     ...
     */
    {
        ...
        /*
         * CURLINFO_REDIRECT_COUNT is not exposed by the trimmed BadgeVMS
         * libcurl headers. CURLINFO_EFFECTIVE_URL compiles, but badge testing
         * showed that after a followed redirect it can still report the
         * original requested URL. The header callback therefore records
         * whether an HTTP 3xx + Location response was actually observed.
         */
...
static page_t *page_info_to_page(const page_t *source,
    ...
    strncpy(pg->title, "Page Information", sizeof(pg->title));
    pg->title[sizeof(pg->title) - 1] = 0;

    /*
     * Keep this screen deliberately small and bounded. URL fields are each
     * limited by URL_MAX, so 1536 bytes is comfortably sufficient.
     */
    ...
    const char *effective =
        g_fetch_meta.redirect_seen
            ? "(not reported)"
            : (g_fetch_meta.effective_url[0]
                ? g_fetch_meta.effective_url
                : (requested_url ? requested_url : "(unknown)"));
    const char *ctype =
        g_fetch_meta.content_type[0]
            ? g_fetch_meta.content_type
            : "(not reported)";
    const char *transport_url =
        (effective[0] && effective[0] != '(')
            ? effective
            : (requested_url ? requested_url : "");
    const char *transport =
        (!strncasecmp(transport_url, "https://", 8)) ? "HTTPS" :
        (!strncasecmp(transport_url, "http://", 7))  ? "HTTP"  : "(unknown)";
    ...
    snprintf(text, cap,
             ...
             "Downloaded: %u bytes\n"
             "Redirect observed: %s\n"
             "Redirect count: not available\n"
             "Transport: %s\n\n"
             "Links: %d\n"
             "Forms: %d\n"
             "Actions: %d\n"
             "Cookies in jar: %d / %d\n\n"
             "HTML download limit: %u bytes (64 KiB)\n\n"
             ...
             g_fetch_meta.redirect_seen ? "yes" : "no",
             transport,
             links,
             forms,
             actions,
    ...
    printf("[mini_browser] page info: status=%ld bytes=%u redirects=na "
           "redirect_seen=%s content_type=%s cookies=%d links=%d forms=%d actions=%d "
           "requested=%s final=%s\n",
           http_status,
           (unsigned)g_fetch_meta.downloaded_bytes,
           g_fetch_meta.redirect_seen ? "yes" : "no",
