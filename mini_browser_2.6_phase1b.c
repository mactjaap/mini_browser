...
/* 2.6 Phase 1B: zero-width RGB color markers encoded as Unicode PUA codepoints. */
#define TEXT_COLOR_R_BASE 0xE000u
#define TEXT_COLOR_G_BASE 0xE100u
#define TEXT_COLOR_B_BASE 0xE200u
#define TEXT_COLOR_POP    0xE300u
#define TEXT_COLOR_INHERIT 0xE301u
...
static void append_color_push(char *out, size_t cap, size_t *used, unsigned char r, unsigned char g, unsigned char b) {
    append_utf8_cp(out,cap,used,TEXT_COLOR_R_BASE+r); append_utf8_cp(out,cap,used,TEXT_COLOR_G_BASE+g); append_utf8_cp(out,cap,used,TEXT_COLOR_B_BASE+b);
}
...
static int wrap_glyph_width(unsigned cp) {
    if (cp == TEXT_BOLD_ON || cp == TEXT_BOLD_OFF ||
        ...
        (cp >= TEXT_COLOR_R_BASE && cp <= TEXT_COLOR_R_BASE + 255) ||
        (cp >= TEXT_COLOR_G_BASE && cp <= TEXT_COLOR_G_BASE + 255) ||
        (cp >= TEXT_COLOR_B_BASE && cp <= TEXT_COLOR_B_BASE + 255) ||
...
static void draw_text(SDL_Renderer *r, int x, int y, const char *s, int max_w) {
    ...
    int pending_rgb = 0; unsigned char pending_r=0, pending_g=0;
    ...
    while (i <= L) {
        ...
        while (i < L) {
            ...
            if (cp >= TEXT_COLOR_R_BASE && cp <= TEXT_COLOR_R_BASE + 255) { pending_r=(unsigned char)(cp-TEXT_COLOR_R_BASE); pending_rgb=1; continue; }
            if (cp >= TEXT_COLOR_G_BASE && cp <= TEXT_COLOR_G_BASE + 255 && pending_rgb==1) { pending_g=(unsigned char)(cp-TEXT_COLOR_G_BASE); pending_rgb=2; continue; }
            if (cp >= TEXT_COLOR_B_BASE && cp <= TEXT_COLOR_B_BASE + 255 && pending_rgb==2) {
                if (color_depth < 32) { color_stack[color_depth].custom=custom_color; color_stack[color_depth].r=custom_r; color_stack[color_depth].g=custom_g; color_stack[color_depth].b=custom_b; color_depth++; }
                custom_color=true; custom_r=pending_r; custom_g=pending_g; custom_b=(unsigned char)(cp-TEXT_COLOR_B_BASE); pending_rgb=0; continue;
            }
            if (cp == TEXT_COLOR_INHERIT) {
                if (color_depth < 32) { color_stack[color_depth].custom=custom_color; color_stack[color_depth].r=custom_r; color_stack[color_depth].g=custom_g; color_stack[color_depth].b=custom_b; color_depth++; }
                pending_rgb=0; continue;
            }
            if (cp == TEXT_COLOR_POP) {
                if (color_depth > 0) { color_depth--; custom_color=color_stack[color_depth].custom; custom_r=color_stack[color_depth].r; custom_g=color_stack[color_depth].g; custom_b=color_stack[color_depth].b; }
                pending_rgb=0; continue;
            }
            if (cp == '\n') {
                saw_newline = true;
