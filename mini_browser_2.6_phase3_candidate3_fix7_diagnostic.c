...
/* ---------- Mini Browser 2.6 Phase 3 Candidate 3 Fix 7 DIAGNOSTIC ---------- */
/*
 ...
 * IMAGE_DECODE_MAX_BYTES.  This prevents a small compressed file from causing
 * an unbounded multi-megabyte RGB allocation.
 */
...
static char *wrap_text(const char *in, int max_cols) {
    printf("[mini_browser][diag5] wrap_text ENTER in=%p max_cols=%d\n",
           (const void *)in, max_cols);
    if (!in) {
        printf("[mini_browser][diag5] wrap_text NULL input\n");
        return NULL;
    }

    printf("[mini_browser][diag5] wrap_text before strlen\n");
    size_t n = strlen(in);
    printf("[mini_browser][diag5] wrap_text after strlen n=%u\n", (unsigned)n);
     ...
     */
    size_t out_bytes = n * 2 + 8;
    printf("[mini_browser][diag5] wrap_text before malloc bytes=%u\n",
           (unsigned)out_bytes);
    char *out = (char*)malloc(out_bytes);
    printf("[mini_browser][diag5] wrap_text after malloc out=%p\n", (void *)out);
    if (!out) {
        printf("[mini_browser][diag5] wrap_text MALLOC FAILED\n");
        return NULL;
    }
    ...
    size_t i = 0;
    size_t o = 0;
    printf("[mini_browser][diag5] wrap_text starting loop n=%u\n", (unsigned)n);
    int line_px = 0;
    int blank_run = 0;
    ...
    while (i < n) {
        if (i == 0 || (i % 128U) == 0)
            printf("[mini_browser][diag5] wrap_text loop i=%u o=%u\n",
    ...
    out[o] = 0;
    printf("[mini_browser][diag5] wrap_text DONE out=%p output_bytes=%u\n",
           (void *)out, (unsigned)o);
    return out;
}
...
/* Fix 6 diagnostic only: observe the current page-text allocation address
 * across each image-loading stage. Never dereference it here. */
static const char *g_diag6_page_text_ptr = NULL;
static size_t g_diag6_page_text_len = 0;
static const page_t *g_diag7_page = NULL;

static void diag7_page_fields(const char *stage) {
    if (!g_diag7_page) {
        printf("[mini_browser][diag7] %-30s pg=(null)\n", stage ? stage : "(null)");
        return;
    }
    printf("[mini_browser][diag7] %-30s pg=%p text=%p template=%p "
           "links=%d actions=%d forms=%d images=%d seen=%d images0=%p src0=%p\n",
           stage ? stage : "(null)",
           (const void *)g_diag7_page,
           (const void *)g_diag7_page->text,
           (const void *)g_diag7_page->text_template,
           g_diag7_page->link_count, g_diag7_page->action_count,
           g_diag7_page->form_count, g_diag7_page->image_count,
           g_diag7_page->image_seen_count,
           (const void *)&g_diag7_page->images[0],
           (const void *)g_diag7_page->images[0].src);
}

static void diag6_text_ptr(const char *stage) {
    printf("[mini_browser][diag6] %-30s page_text=%p known_len=%u\n",
           stage ? stage : "(null)",
           (const void *)g_diag6_page_text_ptr,
           (unsigned)g_diag6_page_text_len);
    diag7_page_fields(stage);
}
...
static int decode_web_image_bounded(const unsigned char *buf, size_t len,
    ...
    diag6_text_ptr("before stbi_info");
    int diag6_info_ok = stbi_info_from_memory(buf, (int)len, &w, &h, &channels);
    printf("[mini_browser][diag6] after stbi_info ok=%d size=%dx%d channels=%d\n",
           diag6_info_ok, w, h, channels);
    diag6_text_ptr("after stbi_info");
    if (!diag6_info_ok) {
    ...
    printf("[mini_browser][diag5] image before stbi_load compressed=%u expected_rgb=%u\n",
           (unsigned)len, (unsigned)(pixels * 3U));
    diag6_text_ptr("before stbi_load");
    unsigned char *rgb =
        stbi_load_from_memory(buf, (int)len, &w, &h, &channels, 3);
    printf("[mini_browser][diag5] image after stbi_load rgb=%p size=%dx%d channels=%d\n",
           (void *)rgb, w, h, channels);
    diag6_text_ptr("after stbi_load");
    if (!rgb) {
    ...
    printf("[mini_browser][diag5] image before RGB565 malloc bytes=%u\n",
           (unsigned)retained_bytes);
    diag6_text_ptr("before RGB565 malloc");
    uint16_t *small = (uint16_t*)malloc(retained_bytes);
    printf("[mini_browser][diag5] image after RGB565 malloc small=%p\n",
           (void *)small);
    diag6_text_ptr("after RGB565 malloc");
    if (!small) {
        stbi_image_free(rgb);
        printf("[mini_browser] image: no memory for %dx%d RGB565 cache\n", dw, dh);
        return 0;
    }

    diag6_text_ptr("before RGB565 downscale");
    printf("[mini_browser][diag6] downscale begin src=%dx%d dst=%dx%d rgb=%p small=%p\n",
           w, h, dw, dh, (void *)rgb, (void *)small);
    for (int y = 0; y < dh; y++) {
        if (y == 0 || y == dh / 2 || y == dh - 1)
            printf("[mini_browser][diag6] downscale row=%d/%d\n", y, dh);
    ...
    printf("[mini_browser][diag6] downscale DONE\n");
    diag6_text_ptr("after RGB565 downscale");
    printf("[mini_browser][diag5] image conversion DONE; before stbi_image_free rgb=%p\n",
           (void *)rgb);
    diag6_text_ptr("before stbi_image_free");
    stbi_image_free(rgb);
    printf("[mini_browser][diag5] image after stbi_image_free\n");
    diag6_text_ptr("after stbi_image_free");
    printf("[mini_browser][diag5] image before decoded_image_release out=%p old_rgb565=%p\n",
           (void *)out, (void *)out->rgb565);
    decoded_image_release(out);
    printf("[mini_browser][diag5] image after decoded_image_release\n");
    out->rgb565 = small;
    out->width = dw;
    out->height = dh;
    out->loaded = true;
...
static int load_image_url(const char *url, int target_max_w, int target_max_h,
    ...
    diag6_text_ptr("before fetch_image_bytes");

    int rc = fetch_image_bytes(url, &m, &status);
    printf("[mini_browser][diag6] after fetch_image_bytes rc=%d status=%ld compressed=%p len=%u\n",
           rc, status, (void *)m.buf, (unsigned)m.len);
    diag6_text_ptr("after fetch_image_bytes");
    if (rc != 0 || status >= 400) {
    ...
    diag6_text_ptr("before decode_web_image");
    int ok = decode_web_image_bounded((const unsigned char*)m.buf, m.len,
                                      target_max_w, target_max_h, out);
    printf("[mini_browser][diag6] after decode_web_image ok=%d retained=%p %dx%d\n",
           ok, (void *)out->rgb565, out->width, out->height);
    diag6_text_ptr("after decode_web_image");
    if (ok) {
    ...
    printf("[mini_browser][diag5] image before free compressed ptr=%p bytes=%u\n",
           (void *)m.buf, (unsigned)m.len);
    diag6_text_ptr("before free compressed");
    free(m.buf);
    printf("[mini_browser][diag5] image after free compressed\n");
    diag6_text_ptr("after free compressed");
    return ok;
}
...
static int draw_decoded_image(SDL_Renderer *r, const page_image_t *spec,
    ...
    printf("[mini_browser][diag] draw_decoded_image ENTER image=%p loaded=%d rgb565=%p size=%dx%d\n",
           (void *)image,
           image ? (int)image->loaded : -1,
           image ? (void *)image->rgb565 : NULL,
           image ? image->width : -1,
           image ? image->height : -1);

    if (!r || !image || !image->loaded || !image->rgb565) {
        printf("[mini_browser][diag] draw_decoded_image EARLY RETURN\n");
        return 0;
    }

    int draw_w = 0, draw_h = 0;
    image_draw_size(spec, image, max_w, max_h, &draw_w, &draw_h);
    printf("[mini_browser][diag] draw_decoded_image draw=%dx%d at %d,%d max=%dx%d\n",
           draw_w, draw_h, x, y, max_w, max_h);

    for (int dy = 0; dy < draw_h; dy++) {
        if (dy == 0 || dy == draw_h / 4 || dy == draw_h / 2 ||
            dy == (draw_h * 3) / 4 || dy == draw_h - 1) {
            printf("[mini_browser][diag] draw row %d/%d\n", dy, draw_h);
        }
    ...
    printf("[mini_browser][diag] draw_decoded_image DONE height=%d\n", draw_h);
    return draw_h;
}
    ...
    while (running) {
            ...
            trim_inplace(url_buf);
   			 ...
   			 debug_utf8("PAGE TEXT", pg->text);
                            ...
                            /* At this point debug_utf8 has just traversed pg->text
                             * successfully, so taking strlen here is safe and gives
                             * Fix 6 a known-good length. The image-stage probes below
                             * never dereference this pointer. */
                            g_diag6_page_text_ptr = pg->text;
                            g_diag6_page_text_len = strlen(pg->text);
                            g_diag7_page = pg;
                            printf("[mini_browser][diag6] captured page_text=%p known_len=%u pg=%p\n",
                                   (const void *)g_diag6_page_text_ptr,
                                   (unsigned)g_diag6_page_text_len,
                                   (void *)pg);
                            diag7_page_fields("captured baseline");

                            printf("[mini_browser][diag] before load_page_images pg=%p images=%d\n",
                                   (void *)pg, pg->image_count);
                            int diag_loaded_images = load_page_images(pg);
                            printf("[mini_browser][diag] after load_page_images loaded=%d\n",
                                   diag_loaded_images);
                            diag7_page_fields("after load_page_images");

    			printf("[mini_browser][diag] before wrap_text\n");
                            diag7_page_fields("immediately before wrap_text");
    			char *wrapped = wrap_text(pg->text, max_cols);
    			printf("[mini_browser][diag] after wrap_text wrapped=%p\n",
                                   (void *)wrapped);

    			debug_utf8("WRAPPED", wrapped);

    			printf("[mini_browser][diag] before replacing content_wrapped old=%p\n",
                                   (void *)content_wrapped);
    			free(content_wrapped);
                            content_wrapped = wrapped;
                            printf("[mini_browser][diag] content_wrapped installed=%p\n",
                                   (void *)content_wrapped);

                            printf("[mini_browser][diag] before free_page old_page=%p\n",
                                   (void *)page);
                            free_page(page);
                            printf("[mini_browser][diag] after free_page old page\n");

                            page = pg;
                            printf("[mini_browser][diag] new page installed=%p images=%d\n",
                                   (void *)page, page ? page->image_count : -1);
                            scroll_lines = 0;
                            sel_action = -1;
...
/* Compose bar text */
            ...
            if (content_wrapped) {
                ...
                while (p && *p && drawn < lines_per_page &&
                    ...
                    if (is_image_marker_line(p, len, &image_index)) {
                        printf("[mini_browser][diag] marker encountered index=%d page=%p y=%d\n",
                               image_index, (void *)page, y);
                        if (image_index >= 0 &&
                            image_index < MAX_INLINE_IMAGES &&
                            g_inline_images[image_index].loaded) {
                            printf("[mini_browser][diag] before inline draw index=%d image=%p rgb565=%p retained=%dx%d\n",
                                   image_index,
                                   (void *)&g_inline_images[image_index],
                                   (void *)g_inline_images[image_index].rgb565,
                                   g_inline_images[image_index].width,
                                   g_inline_images[image_index].height);
                            int image_h = draw_decoded_image(
                                ren, &page->images[image_index],
                                &g_inline_images[image_index],
                                PAD_LR, y, VIEW_W - 2*PAD_LR,
                                IMAGE_DRAW_MAX_H);
                            printf("[mini_browser][diag] after inline draw index=%d image_h=%d\n",
                                   image_index, image_h);
        ...
        printf("[mini_browser][diag] before SDL_RenderPresent\n");
        SDL_RenderPresent(ren);
        printf("[mini_browser][diag] after SDL_RenderPresent\n");
