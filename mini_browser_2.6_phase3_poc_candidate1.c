...
#define IMAGE_DOWNLOAD_MAX    (64 * 1024)
#define IMAGE_SOURCE_MAX_W    160
#define IMAGE_SOURCE_MAX_H    120
#define IMAGE_DRAW_MAX_W      320
#define IMAGE_DRAW_MAX_H      240
#define IMAGE_RESERVE_LINES   8
        ...
        if (!strcmp(tag, "head")) in_head = true;
            ...
            if (tag_attribute(attributes, tag_end, "src",
                ...
                if (g_display_mode == DISPLAY_COLORS_IMAGES &&
                    ...
                    append_line_break(template_text, template_cap, &used);
                    for (int reserve = 1; reserve < IMAGE_RESERVE_LINES; reserve++)
                        append_line_break(template_text, template_cap, &used);
                } else {
...
static int fetch_image_bytes(const char *url, mem_t *m, long *http_status) {
    ...
    hdrs = curl_slist_append(hdrs,
        "Accept: image/x-portable-pixmap,image/*;q=0.5,*/*;q=0.1");
...
static int ppm_skip_ws_comments(const unsigned char *buf, size_t len, size_t *pos) {
    for (;;) {
...
static int ppm_read_uint(const unsigned char *buf, size_t len,
                         size_t *pos, unsigned *value) {
    if (!ppm_skip_ws_comments(buf, len, pos)) return 0;
    unsigned v = 0;
    int digits = 0;
    while (*pos < len && isdigit((unsigned char)buf[*pos])) {
        if (v > 1000000U) return 0;
        v = v * 10U + (unsigned)(buf[*pos] - '0');
        (*pos)++;
        digits++;
    }
    if (!digits) return 0;
...
/* Tiny P6 decoder: 8-bit RGB only, bounded dimensions, no large stack arrays. */
static int decode_ppm_p6(const unsigned char *buf, size_t len,
                         decoded_image_t *out) {
    if (!buf || len < 4 || !out) return 0;
    if (buf[0] != 'P' || buf[1] != '6') return 0;

    size_t pos = 2;
    unsigned w = 0, h = 0, maxval = 0;
    if (!ppm_read_uint(buf, len, &pos, &w) ||
        !ppm_read_uint(buf, len, &pos, &h) ||
        !ppm_read_uint(buf, len, &pos, &maxval))
        return 0;
    ...
    if (pos >= len || !isspace((unsigned char)buf[pos]))
        return 0;
    pos++; /* exactly one required separator before binary raster */

    size_t pixels = (size_t)w * (size_t)h;
    if (pixels > (size_t)IMAGE_SOURCE_MAX_W * IMAGE_SOURCE_MAX_H)
        return 0;
...
static int load_page_image(const page_t *page) {
    ...
    int ok = decode_ppm_p6((const unsigned char*)m.buf, m.len,
                           &g_decoded_image);
    if (ok) {
        strncpy(g_decoded_image.url, image->src,
                sizeof(g_decoded_image.url) - 1);
        printf("[mini_browser] image: loaded PPM %dx%d, %u bytes\n",
               g_decoded_image.width, g_decoded_image.height,
               (unsigned)m.len);
    } else {
        printf("[mini_browser] image: unsupported/invalid PPM\n");
    }
...
static void draw_decoded_image(SDL_Renderer *r, int x, int y, int max_w) {
    ...
    if (draw_w > IMAGE_DRAW_MAX_W) {
        draw_h = (draw_h * IMAGE_DRAW_MAX_W) / draw_w;
        draw_w = IMAGE_DRAW_MAX_W;
    }
    if (draw_h > IMAGE_DRAW_MAX_H) {
        draw_w = (draw_w * IMAGE_DRAW_MAX_H) / draw_h;
    ...
    for (int dy = 0; dy < draw_h; dy++) {
        ...
        for (int dx = 0; dx <= draw_w; dx++) {
            ...
            if (dx < draw_w) {
                ...
                size_t off = ((size_t)sy * src_w + sx) * 3U;
...
/* Compose bar text */
        ...
        if (options_open) {
            ...
            oy += (CH_H + LINE_SPACING);
            draw_text(ren, PAD_LR, oy, "1 image/page, PPM P6, max 160x120", VIEW_W - 2*PAD_LR);
            oy += (CH_H + LINE_SPACING);
                ...
                while (p && *p && drawn < lines_per_page) {
                    ...
                    if (is_image_marker_line(p, len, &image_index)) {
                        if (image_index == 0 && g_decoded_image.loaded)
                            draw_decoded_image(ren, PAD_LR, y, VIEW_W - 2*PAD_LR);
                        else
