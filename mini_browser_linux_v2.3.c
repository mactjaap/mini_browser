...
/*
 * Mini Browser 2.3 - Linux text-only edition
 *
 * Linux companion to the WHY2025 BadgeVMS Mini Browser 2.3.
...
static bool has_scheme(const char *u){return u&&(!strncasecmp(u,"http://",7)||!strncasecmp(u,"https://",8));}
static bool http_url(const char *u){return u&&(!strncmp(u,"http://",7)||!strncmp(u,"https://",8));}
static void normalize_url(char *u){
    trim(u);
    if(u[0]&&!has_scheme(u)){
        char t[URL_MAX];snprintf(t,sizeof t,"%s",u);
        if(strlen(t)+8>=URL_MAX){u[0]=0;return;}
        snprintf(u,URL_MAX,"https://%s",t);
    }
}
...
static void resolve_url(const char*base,const char*href,char*out,size_t cap){
    if(!href||!*href){*out=0;return;}
    if(!strncasecmp(href,"http://",7)||!strncasecmp(href,"https://",8)){
        snprintf(out,cap,"%s",href);return;
    }
    ...
    char d[URL_MAX];directory(base,d,sizeof d);snprintf(out,cap,"%s%s",d,href);
}
...
static int emit_utf8(unsigned long cp,char*o,size_t*u,size_t cap){
    if(cp>0x10FFFFUL||(cp>=0xD800UL&&cp<=0xDFFFUL))cp=0xFFFD;
    if(cp<=0x7F){if(*u+1>=cap)return 0;o[(*u)++]=(char)cp;}
    else if(cp<=0x7FF){if(*u+2>=cap)return 0;o[(*u)++]=(char)(0xC0|(cp>>6));o[(*u)++]=(char)(0x80|(cp&0x3F));}
    else if(cp<=0xFFFF){if(*u+3>=cap)return 0;o[(*u)++]=(char)(0xE0|(cp>>12));o[(*u)++]=(char)(0x80|((cp>>6)&0x3F));o[(*u)++]=(char)(0x80|(cp&0x3F));}
    else{if(*u+4>=cap)return 0;o[(*u)++]=(char)(0xF0|(cp>>18));o[(*u)++]=(char)(0x80|((cp>>12)&0x3F));o[(*u)++]=(char)(0x80|((cp>>6)&0x3F));o[(*u)++]=(char)(0x80|(cp&0x3F));}
    o[*u]=0;return 1;
}
static const char *decode_entity(const char*h,char*o,size_t*u,size_t cap){
    struct ent{const char*n;char c;} es[]={{"&amp;",'&'},{"&lt;",'<'},{"&gt;",'>'},{"&quot;",'"'},{"&#39;",'\''},{"&apos;",'\''},{"&nbsp;",' '}};
    for(size_t i=0;i<sizeof es/sizeof es[0];i++){size_t n=strlen(es[i].n);if(!strncmp(h,es[i].n,n)){if(*u+1<cap){o[(*u)++]=es[i].c;o[*u]=0;}return h+n;}}
    if(h[0]=='&'&&h[1]=='#'){const char*p=h+2;int base=10;if(*p=='x'||*p=='X'){base=16;p++;}const char*d=p;unsigned long v=0;while(*p&&*p!=';'){int x;if(isdigit((unsigned char)*p))x=*p-'0';else if(base==16&&*p>='a'&&*p<='f')x=10+*p-'a';else if(base==16&&*p>='A'&&*p<='F')x=10+*p-'A';else return NULL;if(x>=base||v>(ULONG_MAX-(unsigned)x)/(unsigned)base)return NULL;v=v*base+(unsigned)x;p++;}if(p!=d&&*p==';'&&emit_utf8(v,o,u,cap))return p+1;}
    return NULL;
}
#define TEXT_BOLD_ON  0x03
#define TEXT_BOLD_OFF 0x04
...
static page_t *html_to_page(const char*html,const char*base){
    ...
    for(const char*cur=html;cur<end&&*cur;){
        ...
        if(closing){if(!strcmp(tag,"head"))head=false;else if(!strcmp(tag,"script"))script=false;else if(!strcmp(tag,"style"))style=false;else if(!strcmp(tag,"form")){cf=-1;newline(t,cap,&u);}else if(!strcmp(tag,"pre")){pre=false;newline(t,cap,&u);}else if(!strcmp(tag,"ol")){if(ol>0)ol--;}else if(!strcmp(tag,"code"))appends(t,cap,&u,"`");else if(!strcmp(tag,"strong")||!strcmp(tag,"b")){char m[2]={TEXT_BOLD_OFF,0};appends(t,cap,&u,m);}else if(!strcmp(tag,"em")||!strcmp(tag,"i"))appends(t,cap,&u,"_");else if(!strcmp(tag,"p")||!strcmp(tag,"div")||!strcmp(tag,"section")||!strcmp(tag,"article")||!strcmp(tag,"main")||!strcmp(tag,"header")||!strcmp(tag,"footer")||!strcmp(tag,"nav")||!strcmp(tag,"aside")||!strcmp(tag,"blockquote")||!strcmp(tag,"address")||!strcmp(tag,"li")||!strcmp(tag,"h1")||!strcmp(tag,"h2")||!strcmp(tag,"h3")||!strcmp(tag,"h4")||!strcmp(tag,"h5")||!strcmp(tag,"h6")||!strcmp(tag,"tr")||!strcmp(tag,"table"))newline(t,cap,&u);cur=after;continue;}
        ...
        else if(!strcmp(tag,"code"))appends(t,cap,&u,"`");else if(!strcmp(tag,"strong")||!strcmp(tag,"b")){char m[2]={TEXT_BOLD_ON,0};appends(t,cap,&u,m);}else if(!strcmp(tag,"em")||!strcmp(tag,"i"))appends(t,cap,&u,"_");else if(!strcmp(tag,"hr")){newline(t,cap,&u);appends(t,cap,&u,"--------------------------------\n");}else if(!strcmp(tag,"td")||!strcmp(tag,"th")){if(u&&t[u-1]!='\n'&&t[u-1]!=' ')appends(t,cap,&u," | ");}
...
static int build_get(const page_t*p,int fi,int submit,char*out,size_t cap){
    if(fi<0||fi>=p->form_count)return 0;
    const form_t*f=&p->forms[fi];
    if(strcasecmp(f->method[0]?f->method:"get","get"))return-1;
    if(f->action[0])resolve_url(p->base,f->action,out,cap);else snprintf(out,cap,"%s",p->base);
    char*h=strchr(out,'#');if(h)*h=0;
    bool has=strchr(out,'?')!=NULL,first=true;
    for(int i=0;i<f->field_count;i++){
        const form_field_t*x=&f->fields[i];
        if(x->disabled||!x->name[0])continue;
        bool sub=!strcmp(x->type,"submit");
        bool ok=!strcmp(x->type,"text")||!strcmp(x->type,"search")||!strcmp(x->type,"url")||!strcmp(x->type,"hidden")||(sub&&i==submit);
        if(!ok)continue;
        char n[193],v[FORM_VALUE_MAX*3+1];
        if(!urlencode(x->name,n,sizeof n)||!urlencode(x->value,v,sizeof v))return 0;
        size_t l=strlen(out);char sep[2]={0};
        if(first){if(!l||(out[l-1]!='?'&&out[l-1]!='&'))sep[0]=has?'&':'?';}
        else if(l&&out[l-1]!='?'&&out[l-1]!='&')sep[0]='&';
        if(!scat(out,cap,sep)||!scat(out,cap,n)||!scat(out,cap,"=")||!scat(out,cap,v))return 0;
        first=false;has=true;
    }
    return 1;
}
static int fetch(const char*url,mem_t*m,long*status,char*effective,size_t ec){
    CURL*c=curl_easy_init();if(!c)return CURLE_FAILED_INIT;m->buf=NULL;m->len=0;*status=0;effective[0]=0;curl_easy_setopt(c,CURLOPT_URL,url);curl_easy_setopt(c,CURLOPT_FOLLOWLOCATION,1L);curl_easy_setopt(c,CURLOPT_MAXREDIRS,5L);curl_easy_setopt(c,CURLOPT_CONNECTTIMEOUT,20L);curl_easy_setopt(c,CURLOPT_TIMEOUT,35L);curl_easy_setopt(c,CURLOPT_LOW_SPEED_LIMIT,10L);curl_easy_setopt(c,CURLOPT_LOW_SPEED_TIME,20L);curl_easy_setopt(c,CURLOPT_WRITEFUNCTION,write_cb);curl_easy_setopt(c,CURLOPT_WRITEDATA,m);curl_easy_setopt(c,CURLOPT_USERAGENT,"Mozilla/5.0 (Linux; rv:2.3) Gecko/20100101 (compatible; MiniBrowser/2.3; +https://github.com/mactjaap/mini_browser/)");
...
static char *wrap(const char*in,int cols){
    size_t n=strlen(in),cap=n+n/(cols?cols:1)+64,o=0,i=0;char*out=malloc(cap);if(!out)return NULL;
    int col=0,blank=0;mbstate_t st;memset(&st,0,sizeof st);
    while(i<n){
        unsigned char c=(unsigned char)in[i];
        if(c==TEXT_BOLD_ON||c==TEXT_BOLD_OFF){if(o+1>=cap){cap*=2;char*p=realloc(out,cap);if(!p){free(out);return NULL;}out=p;}out[o++]=(char)c;i++;continue;}
        if(c=='\r'){i++;continue;}
        if(c=='\n'){if(col==0){if(blank){i++;continue;}blank=1;}else blank=0;if(o+1>=cap){cap*=2;char*p=realloc(out,cap);if(!p){free(out);return NULL;}out=p;}out[o++]='\n';col=0;i++;memset(&st,0,sizeof st);continue;}
        if(c==' '&&(col==0||(o&&out[o-1]==' '))){i++;continue;}

        wchar_t wc;size_t clen=mbrtowc(&wc,in+i,n-i,&st);int width=1;
        if(clen==(size_t)-1||clen==(size_t)-2||clen==0){memset(&st,0,sizeof st);clen=1;width=1;}
        else{int w=wcwidth(wc);width=w>0?w:0;}

        if(cols&&col>0&&col+width>cols){if(o+1>=cap){cap*=2;char*p=realloc(out,cap);if(!p){free(out);return NULL;}out=p;}out[o++]='\n';col=0;if(c==' '){i+=clen;continue;}}
        if(o+clen+1>=cap){while(o+clen+1>=cap)cap*=2;char*p=realloc(out,cap);if(!p){free(out);return NULL;}out=p;}
        memcpy(out+o,in+i,clen);o+=clen;i+=clen;col+=width;blank=0;
    }
    out[o]=0;return out;
}
static void print_formatted(const char*s){
    bool tty=isatty(STDOUT_FILENO),bold=false;
    for(const unsigned char*p=(const unsigned char*)s;p&&*p;p++){
        if(*p==TEXT_BOLD_ON){if(tty&&!bold)fputs("\033[1m",stdout);bold=true;continue;}
        if(*p==TEXT_BOLD_OFF){if(tty&&bold)fputs("\033[22m",stdout);bold=false;continue;}
        putchar(*p);
    }
    if(tty&&bold)fputs("\033[22m",stdout);
}
...
static int activate(page_t*p,int ai,char*nav,size_t nc){
    if(!p||ai<0||ai>=p->action_count)return 0;
    page_action_t*a=&p->actions[ai];
    if(a->type==ACTION_LINK){
        const char*href=p->links[a->link_index].href;
        const char*g1="http://www.google.com/url?q=",*g2="https://www.google.com/url?q=",*dest=NULL;
        if(!strncmp(href,g1,strlen(g1)))dest=href+strlen(g1);
        else if(!strncmp(href,g2,strlen(g2)))dest=href+strlen(g2);
        if(dest){
            size_t len=strlen(dest);const char*end=strstr(dest,"&amp;");if(!end)end=strchr(dest,'&');if(end)len=(size_t)(end-dest);
            if(len>=nc)len=nc-1;
            memcpy(nav,dest,len);nav[len]=0;
        }else snprintf(nav,nc,"%s",href);
        return 1;
    }
    form_t*f=&p->forms[a->form_index];form_field_t*x=&f->fields[a->field_index];
    if(a->type==ACTION_FORM_FIELD){char b[FORM_VALUE_MAX];printf("%s [%s]: ",x->name,x->value);fflush(stdout);if(!fgets(b,sizeof b,stdin))return 0;b[strcspn(b,"\r\n")]=0;snprintf(x->value,sizeof x->value,"%s",b);if(!refresh_text(p))puts("Could not refresh form display.");return 2;}
    int r=build_get(p,a->form_index,a->field_index,nav,nc);if(r<0){puts("POST FORMS NOT SUPPORTED");return 0;}if(!r){puts("FORM URL TOO LONG");return 0;}return 1;
}
...
int main(int argc,char**argv){
    ...
    while(1){printf("\nmini-browser> ");fflush(stdout);if(!fgets(cmd,sizeof cmd,stdin))break;cmd[strcspn(cmd,"\r\n")]=0;trim(cmd);if(!cmd[0])continue;
        ...
        else if(!strncmp(cmd,"g ",2)){snprintf(requested,sizeof requested,"%.*s",(int)sizeof(requested)-1,cmd+2);normalize_url(requested);if(requested[0])load(requested,&page,current,sizeof current,&status,true);}
